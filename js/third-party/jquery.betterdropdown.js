...
function buildDropdown(originalDropdown, settings) {
    ...
    var $dropdownBox =
        $('<div class="' + className + '-dropdown-box-wrapper dropdown-box-wrapper ' + id + '">\
                <span class="' + className + '-dropdown-box-text dropdown-box-text">--Select--</span>\
            </div>');
    ...
    $.each($('#' + id).children(), function (i, option) {
        ...
        if (option.value !== '') {
            ...
            var $filterResult = $('<div id= "dropdown' + option.value.split("~")[0] + '" class="' + className + '-result result"></div>');
            $filterResult
                ...
                .click(function () {
                    selectItem($filterResult, className, id, $filterWrapper);
                })
                .mouseover(function () {
            ...
            if ($(option).data('displayBelowText')) {
                if (typeof ($(option).data('displayBelowText')) == 'number') {
                    $filterResult.data('filter-below-text', $(option).data('displayBelowText'));
                } else {
    ...
    $('.' + className + '-filter-box').keyup(function (e) {
        ...
        if (e.keyCode !== 13 &&
            ...
            e.keyCode !== 39 &&
            e.keyCode !== 40) {
            filterItems($('.' + className + '-filter-box').val().toUpperCase(), className, itemName);
        }
    });

    $('.' + className + '-filter-wrapper').keydown(function (e) {
        arrowKeyNavigate(e, className, $filterResultsWrapper, $filterWrapper, id);
...
function selectItem($item, className, id, $filterWrapper) {
    ...
    if ($item.data()) {
        ...
        $('#' + id + ' option[value="--Select Workspace--"]').prop("selected", true);
    }
    $('#' + id).trigger("change");
    ...
    closeDropdown($filterWrapper, className);
    if ($item.length != 0 && !$("#complex_functions_container")?.length != 0) {
        load()
    }
}
...
function filterItems(filterText, className, itemName) {
...
function toggleDropdown(className) {
    ...
    if ($filterWrapper.css('display') === 'none') {
        ...
        if ($selectedItem.length > 0) {
            scrollFilterResultsWrapper($selectedItem, className);
        }
...
function closeDropdown($filterWrapper, className) {
    if (!$filterWrapper) {
        return;
    }
    $filterWrapper
...
function flipDropdownArrow(className) {
...
function arrowKeyNavigate(e, className, $filterResultsWrapper, $filterWrapper, id) {
    ...
    switch (e.which) {
        ...
        case 13:
            ...
            if ($hoveredItem.length > 0) {
                selectItem($hoveredItem, className, id, $filterWrapper);
            }
            break;
    }
    ...
    function upOrDown($item, $sibling, $wrapToItem) {
        if ($item.length > 0) {
            if ($sibling.length > 0) {
                hoverItem($sibling);
                scrollFilterResultsWrapper($sibling, className);
            } else {
                hoverItem($wrapToItem);
                scrollFilterResultsWrapper($wrapToItem, className);
            }
        } else {
            hoverItem($firstItem);
            scrollFilterResultsWrapper($firstItem, className);
        }
    }
}
function scrollFilterResultsWrapper($hoveredItem, className) {
...
window.addEventListener('click', function (e) {
    if (!document.getElementById('workspace_container').contains(e.target) && !document.getElementById('complex_functions_container')?.contains(e.target)) {
        // The classname is wrong but this works?
        closeDropdown($(".filter-wrapper"), 1661096919455);
    }
});